...
func input_process(delta : float):
	...
	if (!is_in_photo_mode and !garbage_bag_handler.is_holding_a_bag and garbage_bag_raycast.is_colliding()):
		if (Input.is_action_just_pressed("player_action_primary")):
			...
			return
		UI.instance.ui_interaction_tooltip.show_tooltip = true
	else:
		UI.instance.ui_interaction_tooltip.show_tooltip = false
